...
class SpeechMixEED(nn.Module):
    ...
    def __init__(self,
        ...
        self.device = torch.device(
            "cuda" if torch.cuda.is_available() else "cpu")
        self.encoder_model = getattr(hub,
                                     speech_model_config)().to(self.device)
        ...
        self.encoder_model.model.encoder.layers = (
            ...
            layers[:len(self.encoder_model.model.encoder.layers) -
                   remove_layers])
        ...
        if self.downsize > 1:
            self.length_adapters = nn.Sequential(*[
                nn.Conv1d(
                    in_channels=self.encoder_model.model.final_proj.
                    in_features,
                    out_channels=self.encoder_model.model.final_proj.
                    in_features,
                    kernel_size=2,
                    stride=2,
        ...
        self.weights_sum = nn.Parameter(
            torch.zeros(self.num_speech_encoder_layers)).to(self.device)
    ...
    def forward(
        ...
        if input_text_prompt is not None:
            text_prompt = self.nlp_emb(
                self.tokenizer(input_text_prompt,
...
class SpeechMixEEDT5eval(SpeechMixEEDT5):
    ...
    def forward(
        ...
        if decoder_text_prompt is not None:
            text_prompt_emds = self.nlp_emb(decoder_text_prompt.to(
                self.device))
